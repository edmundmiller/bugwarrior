...
from bugwarrior.services.applereminders import (
    AppleRemindersConfig, AppleRemindersService, AppleRemindersClient, AppleRemindersIssue
)
...
class TestAppleRemindersIssue(AbstractServiceTest, ServiceTest):
    ...
    def test_to_taskwarrior_completed(self):
        ...
        service = self.get_mock_service(AppleRemindersService)
        issue = service.get_issue_for_record(COMPLETED_REMINDER, {'project': 'Work', 'annotations': []})
    ...
    def test_to_taskwarrior_priority_mapping(self):
        ...
        issue = service.get_issue_for_record(HIGH_PRIORITY_REMINDER, {'project': 'Work', 'annotations': []})
        ...
        issue = service.get_issue_for_record(LOW_PRIORITY_REMINDER, {'project': 'Personal', 'annotations': []})
        self.assertEqual(issue.to_taskwarrior()['priority'], 'L')
        
        # Test no priority (0 -> None, falls back to service default)
        issue = service.get_issue_for_record(NO_PRIORITY_REMINDER, {'project': 'Personal', 'annotations': []})
...
class TestAppleRemindersClient(ServiceTest):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_init_success(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_init_with_config(self, mock_apple_reminders):
    ...
    def test_init_missing_library(self):
        ...
        with patch('bugwarrior.services.applereminders.apple_reminders', side_effect=ImportError):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_init_connection_error(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminder_lists_success(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminder_lists_error(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_no_lists_configured(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_specific_lists(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_exclude_lists(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_include_completed(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_exclude_completed(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_due_only(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_get_reminders_no_matching_lists(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_reminder_to_dict_success(self, mock_apple_reminders):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_reminder_to_dict_error_handling(self, mock_apple_reminders):
        ...
        # Create a mock reminder that raises an exception
        mock_reminder = Mock()
...
class TestAppleRemindersService(AbstractServiceTest, ServiceTest):
    ...
    def test_to_taskwarrior(self):
        ...
        expected = {
            'annotations': [],
            'description': '(bw)task#test-reminder-123 - Buy groceries .. x-apple-reminderkit://REMCDReminder/test-reminder-123',
            'due': ARBITRARY_DUE,
            'entry': ARBITRARY_CREATED,
            'end': None,
            'modified': ARBITRARY_MODIFIED,
    ...
    def test_service_initialization_with_config(self):
        ...
        # Note: This test verifies the service passes config to client correctly
...
class TestAppleRemindersIntegration(ServiceTest):
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_end_to_end_workflow(self, mock_apple_reminders):
        """Test complete workflow from service to task construction."""
        # Set up mock Apple Reminders app
        mock_reminders_app = MockRemindersApp({
            'Work': [HIGH_PRIORITY_REMINDER, ARBITRARY_REMINDER],
            'Personal': [LOW_PRIORITY_REMINDER],  # Should be filtered out by lists config
        })
        mock_apple_reminders.RemindersApp.return_value = mock_reminders_app
        ...
        # Should only get reminders from Work list
        self.assertEqual(len(issues), 2)
    ...
    @patch('bugwarrior.services.applereminders.apple_reminders')
    def test_priority_mapping_integration(self, mock_apple_reminders):
        ...
        self.assertIn(service.config.default_priority, priorities)  # Default for no priority